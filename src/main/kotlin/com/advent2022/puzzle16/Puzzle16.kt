...
class Path(val actions: ArrayDeque<Action> = ArrayDeque(), val openedValves: HashSet<String> = HashSet()) {
    ...
    private fun pathWithAction(actionType: ActionType, valve: Valve, step: Int) =
        Path(
            ArrayDeque<Action>().also { it.add(Action(actionType, step, valve)) },
...
class Puzzle16 : Base<Data, Solution?, Solution2?>() {
    ...
    private fun findBestPath(valve: Valve, thisPath: Path, numStepsLeft: Int, data: Data): Path {
        val bestChild: Path? = when {
            numStepsLeft in 0..1 -> null
            numStepsLeft == 2 -> thisPath.open(valve, numStepsLeft) ?: thisPath.move(valve, numStepsLeft)
            valve.children.isEmpty() -> null
            else -> {
                valve.bestFlowRate.getOrPut(numStepsLeft) {
                    var currentBest: Path? = null
                    ...
                    val bestPath = bestPath(currentBest, bestChildrenPath)
                    bestPath!!
                }
            }
        }

        return if (bestChild != null) thisPath.add(bestChild) else thisPath
    }
    ...
    private fun bestPath(path1: Path?, path2: Path?) =
        when {
            path1 == null -> path2
            path2 == null -> path1
            path1.score > path2.score -> path1
            else -> path2
        }
